...
async function displayAlbums() {
    ...
    let folder = urlPath.split("/").filter(Boolean).pop();

    // for to fix error coming in console this if condition used   (from chatgpt)
   if (!folder || folder === "songs" || folder.includes(".") || e.href.match(/\.(mp3|jpg|jpeg|png|json|txt|svg)$/i)) {
        continue;  // Skip files
    }
...
async function main() {
    ...
    document.querySelector(".seekbar").addEventListener("click", e => {
        ...
        // chatgpt says to write this instead of above line.
